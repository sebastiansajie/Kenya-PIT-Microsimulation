...
@iterate_jit(nopython=True)
def cal_pit(pitax_w, total_reliefs, pitax):
    """
    Compute Total PIT.
    """
    pitax = max(0, pitax_w - total_reliefs)
    return pitax

@iterate_jit(nopython=True)
def cal_total_pit(pitax_w, pitax_c, total_reliefs, pitax_total):
    """
    Compute Total PIT.
    """
    pitax_total = max(0, pitax_w + pitax_c - total_reliefs)
    return pitax_total